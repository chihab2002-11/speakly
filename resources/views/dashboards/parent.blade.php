<x-layouts::app :title="__('Parent Dashboard')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <flux:card class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <flux:heading size="lg">{{ __('Parent Dashboard') }}</flux:heading>
                <flux:text class="mt-1">{{ __('Overview of your conversations and notifications.') }}</flux:text>
            </div>
            <div class="inline-flex items-center rounded-full bg-purple-50 px-3 py-1 text-sm font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">
