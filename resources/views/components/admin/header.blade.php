...
<header class="sticky top-0 z-30 flex items-center justify-between gap-4 border-b px-4 py-3 md:px-8" style="background: rgba(255, 255, 255, 0.85); border-color: rgba(226, 232, 240, 0.5); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); box-shadow: 0px 1px 2px rgba(0, 0, 0, 0.05);">
    ...
    <div class="flex items-center gap-3 md:gap-5">
        ...
        <a href="{{ route('admin.notifications') }}" class="relative inline-flex h-9 w-9 items-center justify-center rounded-full transition-all duration-200 hover:bg-gray-100 active:scale-95" data-live-notification-bell>
            ...
            <span data-live-notification-count class="{{ $unreadNotificationsCount > 0 ? '' : 'hidden' }} absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">{{ $unreadNotificationsCount > 9 ? '9+' : $unreadNotificationsCount }}</span>
        </a>

        <a href="{{ route('support') }}" class="inline-flex h-9 w-9 items-center justify-center rounded-full transition-all duration-200 hover:bg-gray-100 active:scale-95" aria-label="Support">
        ...
        <a href="{{ route('admin.settings') }}" class="inline-flex h-9 w-9 items-center justify-center rounded-full transition-all duration-200 hover:bg-gray-100 active:scale-95" aria-label="Admin settings">
