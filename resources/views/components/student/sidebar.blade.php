...
@php
    ...
    try {
        if ($user && method_exists($user, 'enrolledClasses')) {
            ...
            $attendanceClassIds = \App\Models\AttendanceRecord::query()
            ...
            if ($classIds->isNotEmpty()) {
                $resourcesQuery = \App\Models\TeacherResource::query()->whereIn('class_id', $classIds);
                $hasMaterialsUpload = $resourcesQuery->exists();

                $latest = (clone $resourcesQuery)->max('created_at');
                $materialsLatestUploadTs = $latest ? (int) (strtotime((string) $latest) * 1000) : 0;
            }
        }
    } catch (\Throwable $e) {
        $hasMaterialsUpload = false;
    ...
    @foreach($navItems as $item)
        ...
        <div class="relative">
            ...
            <a 
                ...
                class="group relative sidebar-link flex items-center gap-4 px-4 py-3 text-sm transition-all duration-200 {{ $isActive ? 'rounded-l-2xl font-bold' : 'rounded-xl mr-4 hover:bg-white/30 font-medium' }}"
                @if($isActive)
            ...
            >
                ...
                @if($item['route'] === 'student.materials' && $hasMaterialsUpload)
                    <span id="studentMaterialsSidebarBadge" data-latest-upload-ts="{{ $materialsLatestUploadTs }}" class="ml-1 hidden h-2.5 w-2.5 rounded-full bg-red-500" aria-label="New learning material"></span>
                @endif
            </a>
...
<script>
    (function () {
        const badge = document.getElementById('studentMaterialsSidebarBadge');
        const storageKey = 'student-materials-last-seen-upload-ts';

        if (!badge) {
            return;
        }

        function isOnMaterialsPage() {
            const normalizedPath = String(window.location.pathname || '').replace(/\/+$/, '').toLowerCase();

            return normalizedPath.endsWith('/student/materials');
        }

        function markMaterialsAsSeenIfNeeded() {
            if (!isOnMaterialsPage()) {
                return;
            }

            const latestUploadTs = Number(badge.dataset.latestUploadTs || 0);
            ...
            if (latestUploadTs > seenTs) {
                localStorage.setItem(storageKey, String(latestUploadTs));
            }
        }
        ...
        function refreshBadgeState() {
            ...
            if (latestUploadTs > 0 && latestUploadTs > seenTs) {
        ...
        function syncBadgeState() {
            markMaterialsAsSeenIfNeeded();
            refreshBadgeState();
        }

        syncBadgeState();
        window.addEventListener('student-materials-badge:update', syncBadgeState);
        window.addEventListener('popstate', syncBadgeState);
