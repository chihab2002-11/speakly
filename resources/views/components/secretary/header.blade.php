...
>
    <div class="flex items-center gap-4">
        <button
            type="button"
            onclick="toggleSecretarySidebar()"
            ...
            aria-label="Toggle secretary sidebar"
        >
    ...
    <div class="flex items-center gap-4 md:gap-6">
        ...
        <div class="flex items-center gap-2 md:gap-4">
            <a
                href="{{ route('secretary.notifications') }}"
                ...
                data-live-notification-bell
            >
                <svg class="h-5 w-5" fill="none" stroke="currentColor" style="color: var(--lumina-text-muted);" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                </svg>
                <span data-live-notification-count class="{{ $unreadNotificationsCount > 0 ? '' : 'hidden' }} absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold text-white" style="background-color: var(--lumina-accent-red);">
                    {{ $unreadNotificationsCount > 9 ? '9+' : $unreadNotificationsCount }}
                </span>
            </a>

            <a href="{{ route('support') }}" class="flex h-9 w-9 items-center justify-center rounded-full transition-all duration-200 hover:bg-gray-100 active:scale-95" aria-label="Support">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" style="color: var(--lumina-text-muted);" viewBox="0 0 24 24">
