    ...
    <div class="flex items-center gap-4 md:gap-6">
        ...
        <div class="flex items-center gap-2 md:gap-4">
            ...
            >
                ...
                <span data-live-notification-count class="{{ $unreadNotificationsCount > 0 ? '' : 'hidden' }} absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold text-white animate-pulse" style="background-color: var(--lumina-accent-red);">
                    {{ $unreadNotificationsCount > 9 ? '9+' : $unreadNotificationsCount }}
                </span>
            </a>

            {{-- Help Button --}}
            <a href="{{ route('support') }}" class="flex h-9 w-9 items-center justify-center rounded-full transition-all duration-200 hover:bg-gray-100 active:scale-95" aria-label="Support" wire:navigate>
                <svg class="h-5 w-5" fill="none" stroke="currentColor" style="color: var(--lumina-text-muted);" viewBox="0 0 24 24">
