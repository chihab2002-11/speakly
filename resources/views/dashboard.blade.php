    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <flux:card class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                <flux:button :href="route('messages.inbox')" variant="primary" icon="inbox" wire:navigate>
                    {{ __('Open inbox') }}
                </flux:button>
                <flux:button :href="route('messages.create')" variant="ghost" icon="paper-airplane" wire:navigate>
                    {{ __('New message') }}
                </flux:button>
            </div>
        </flux:card>
